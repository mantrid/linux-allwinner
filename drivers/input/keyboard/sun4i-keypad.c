        ...
        if (action & SW_KP_RELEASE) {
            for (row = 0; row < keypad->rows; row++) {
                if (row_state[col] & (1 << row)) {
        			...
        			input_report_key(input_dev, keycodes[val], 0);
                }
            }
            ...
            if (changed) {
                ...
                for (row = 0; row < keypad->rows; row++) {
                    ...
                    val = MATRIX_SCAN_CODE(row, col, keypad->row_shift);
                    input_event(input_dev, EV_MSC, MSC_SCAN, val);
        			input_report_key(input_dev, keycodes[val], press);
                }
            }
...
static int __devinit sw_keypad_probe(struct platform_device *pdev)
{
	...
	keymap_size = (pdata->rows << row_shift);

	keypad = kzalloc(sizeof(*keypad) + keymap_size * sizeof(unsigned short),
			GFP_KERNEL);
	...
	matrix_keypad_build_keymap(keymap_data, NULL, pdata->rows, pdata->cols,
